...
export default function UserIncidents() {
  ...
  const fetchIncidents = async () => {
    try {
      ...
      const res = await getUserIncidents({ per_page: 200, ...(monitorId && { monitor: monitorId }) });
  ...
  const exportToPDF = async () => {
    try {
      ...
      // 2. Full Incident Log (includes Root Cause)
      y = doc.lastAutoTable.finalY + 8;
      section(2, 'Full Incident Log');
      autoTable(doc, {
        startY: y,
        head: [['Monitor', 'Status', 'Cause', 'Root Cause', 'Started', 'Duration']],
        body: data.map(i => [
          i.monitor?.name || 'N/A',
          ...
          i.status === 'open'
            ? 'Ongoing'
            : (i.down_duration_seconds ? formatDurationSecs(i.down_duration_seconds) : 'N/A'),
        ]),
        theme: 'grid',
      ...
      autoTable(doc, {
        ...
        body: Object.entries(causeMap).sort((a, b) => b[1] - a[1]).map(([cat, count]) => [
          cat, count, `${((count / totalCauses) * 100).toFixed(1)}%`,
        ]),
        theme: 'grid',
        headStyles: { fillColor: [240, 240, 240], textColor: BLACK, fontStyle: 'bold', fontSize: 9 },
        bodyStyles: { fontSize: 9, textColor: BLACK },
      ...
      doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(...BLACK);
      [
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        {statusFilter !== 'all' && (
          <span className={`text-xs px-3 py-1 rounded-full font-medium ${
            statusFilter === 'open' ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'
          }`}>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-800 border-b border-gray-700">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Monitor</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Status</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Cause</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Root Cause</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Started</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Resolved</th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-white text-sm">
                      {incident.down_duration_seconds
                        ...
                        : incident.status === 'open' ? 'Ongoing' : 'N/A'}
                    </td>
                  </tr>
                );
              })}
