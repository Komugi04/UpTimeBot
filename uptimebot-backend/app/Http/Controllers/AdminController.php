...
class AdminController extends Controller
{
    ...
    public function getIncidentsByCause(Request $request)
    {
        ...
        $stats = $query->groupBy('error_type')
                       ->orderBy('count', 'desc')
                       ->get();
    ...
    public function getIncidentsByUser(Request $request)
    {
        ...
        switch ($period) {
            case 'hour':
                $query->where('incidents.created_at', '>=', now()->subHour());
                break;
            case 'day':
                $query->where('incidents.created_at', '>=', now()->subDay());
                break;
            case 'week':
        ...
        $stats = $query->groupBy('users.id', 'users.name', 'users.email')
                       ->orderBy('count', 'desc')
                       ->get();
